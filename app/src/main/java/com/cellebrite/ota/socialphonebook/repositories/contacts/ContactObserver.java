...
class ContactObserver extends Thread implements HandlerCreatedObserver {
        ...
        while (b_isAlive) {
            try {
                ...
                if (passedTime >= SLEEP_TIME_BEFORE_NOTIFY_CHANGE) {
                    //notify for change in contacts table
    ...
    private void createAndroidContentObserver(Handler handler) {
        ...
        m_androidContentObserver = new ContentObserver(handler) {
            ...
            public void onChange(boolean selfChange) {
                //#ifdef DEBUG
                Utils.debug("ContactRepositoryObserver.onChange : change in ALL_CONTACTS isSelf =" + selfChange);
                //#endif

                //notify for new change in contacts table
        ...
        cr.registerContentObserver(ContactsContract.Contacts.CONTENT_URI, false, m_androidContentObserver);

    }

    private void notifyChageInContacts() {
        DeviceContactsRepository.getInstance().onChangeInContacts();
    ...
    synchronized void addChange(boolean isSelfChange) {
        ...
        //#ifdef DEBUG
        Utils.debug("ContactChangeTask.addChange() time =  " + m_timeOfLastChange);
        //#endif
    }
